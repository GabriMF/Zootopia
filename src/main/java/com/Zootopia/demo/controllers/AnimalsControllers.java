package com.Zootopia.demo.controllers;
...
import com.Zootopia.demo.service.AnimalsService;

@RestController
@RequestMapping("/api/users")
public class AnimalsControllers {
    @Autowired
    private AnimalsService userService;
    //Creamos nuevo usuario
    @PostMapping // Abreviatura  de RqquestMapping para decir que es un metodo post
    public ResponseEntity<?> create (@RequestBody User user) {
        return ResponseEntity.status(HttpStatus.CREATED).body(AnimalsService.save(user));
    }
    // Leer un usuario
